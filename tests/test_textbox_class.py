...
def main():
    """Test textbox class with persistent state"""
    # Initialize SDL
    ...
    try:
        ...
        ctx = pymui.Context()
        print("Textbox class test - click on textbox first, then type")
        print("Press Escape to exit")

        running = True
        textbox1 = pymui.Textbox(128)
        textbox2 = pymui.Textbox(128)
        text_submissions = 0
        ...
        while running:
            ...
            # Text input test with Textbox class
            if ctx.begin_window("Textbox Class Test", pymui.rect(50, 50, 400, 300)):
                ctx.label("Click on a textbox first, then type:")
                
                # First textbox
                ctx.label("Textbox 1:")
                result, new_text = textbox1.update(ctx)
                if result & pymui.Result.SUBMIT:
                    text_submissions += 1
                    print(f"*** TEXTBOX 1 SUBMITTED! Count: {text_submissions}, Text: '{new_text}' ***")
                
                # Second textbox
                ctx.label("Textbox 2:")
                result, new_text = textbox2.update(ctx)
                if result & pymui.Result.SUBMIT:
                    text_submissions += 1
                    print(f"*** TEXTBOX 2 SUBMITTED! Count: {text_submissions}, Text: '{new_text}' ***")
                
                if ctx.button("Submit All"):
                    text_submissions += 1
                    print(f"*** SUBMIT BUTTON CLICKED! Count: {text_submissions} ***")
                    print(f"Textbox 1: '{textbox1.text}'")
                    print(f"Textbox 2: '{textbox2.text}'")
                
                # Show current values
                ctx.label(f"Textbox 1: '{textbox1.text}'")
                ctx.label(f"Textbox 2: '{textbox2.text}'")
