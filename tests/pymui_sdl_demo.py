...
def log_window(ctx):
    ...
    if ctx.begin_window("Log Window", pymui.rect(350, 40, 300, 200)):
        ...
        # Initialize textbox buffer if not exists
        if not hasattr(log_window, 'text_buf'):
            log_window.text_buf = ""

        # Note: textbox_ex returns (result, new_text)
        result, log_window.text_buf = ctx.textbox_ex(log_window.text_buf, 128)
        ...
        if submitted:
            if log_window.text_buf.strip():
                write_log(log_window.text_buf.strip())
                log_window.text_buf = ""
